...
const getWalletBalance = async (userId: string): Promise<WalletBalance> => {
    const response = await fetch(`${API_BASE_URL}/api/v1/wallets/balance?walletID=${userId}`);
...
const getConvertToARS = async (fromAmount: number, fromToken: string) => {
	const response = await fetch(`${API_BASE_URL}/api/v1/wallets/convert?amount=${fromAmount}&token=${fromToken}`);
	const data = await response.json();
